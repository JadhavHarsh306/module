...
public class LoginDaoImpl implements LoginDao{
	...
	static {
		...
		try {
			...
			seluser=conn.prepareStatement("select user,password,role from user where user=? and password=?");
			adduser=conn.prepareStatement("insert into user(user,password,role) values(?,?,?)");
